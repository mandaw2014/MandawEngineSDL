from mandaw import Mandaw, GameObject, Color
...
class PlatformerController2D(GameObject):
    ...
    def movement(self):
        ...
        if not self.collidelist(self.objects) and self.is_jumping == False:
            ...
            self.velocity_y += 0.1 * self.window.dt
...
if __name__ == "__main__":
    mandaw = Mandaw("PlatformerController2D!", 800, 600, Color(0, 255, 255))

    player = PlatformerController2D(mandaw, 0, 0, True)

    ground = GameObject(mandaw, 5000, 100, 0, 500, Color(150, 150, 150))
    ground.center_x()
